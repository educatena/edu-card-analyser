...
def display(img, frameName="img"):
    ...
    cv2.imwrite(f'debug/display_{frameName}.jpg', img)
...
def main(img, name='rot'):
    display(img, f'{name}_original')
    ...
    for idx in range(len(contours)):
        ...
        # display(mask, f'{name}_mask_{idx}')
    ...
    orientation = (cummTheta/ct)# * -1
    print("Image orientation in degress: ", orientation)
    finalImage = rotate(img, orientation)
    # display(textImg, f'{name}_bounding_box')
    display(finalImage, f'{name}_final_img')
    # cv2.imwrite('/debug/rotated_img.jpg', finalImage)

if __name__ == "__main__":
    

    os.system('rm -dr debug/*')
    filePath = '/home/tetra/Downloads/escaneados/1xb.jpeg'
